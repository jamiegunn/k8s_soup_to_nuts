...
2. **Choose the primary signal.** The knee test tells you *which* number saturated first. If CPU tracked the knee → CPU HPA (below). If threads hit 85% while CPU coasted at 40% → you're wait-bound; scale on the [busy-thread ratio through the pipeline's fork](/autoscaling/getting-the-metrics/#5-the-fork-adapter-or-keda) instead, same floor/ceiling. `payments-api` measured CPU-correlated, so CPU it is.
