...
2. **OOMKilled from default heap + tight limit.** The JVM's default heap slice and Spring's non-heap appetite (Metaspace, code cache, threads) don't leave room for each other in a tight limit — restart counter climbs, exit code 137; the sizing rules are in [The JVM in Containers](/java/jvm-in-containers/).
