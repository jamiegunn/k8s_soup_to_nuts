...
**Why k3s?** Because it isn't a simulator. k3s is a certified Kubernetes distribution that packages the entire control plane and the kubelet into a single binary — the same distribution running on production edge devices, in retail back rooms, and on on-prem racks. The cluster you break on your laptop is, component for component, one you may later be paged for.
