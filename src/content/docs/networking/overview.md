...
When these practical guides aren't deep enough, the [Routing & DNS Deep Dive](/routing/overview/) section traces a request end to end ([Life of a Request](/routing/life-of-a-request/)) and dissects the machinery ([kube-proxy and the dataplane](/routing/kube-proxy-and-the-dataplane/), [CoreDNS](/routing/coredns-deep-dive/), [DNS integration](/routing/dns-integration/)). And when you want the internal fabric these hops ride on — how the pod network, the Service network, and cluster DNS are actually carved up and allocated under the hood — the [Cluster Networking](/cluster-networking/overview/) section is the layer below this one.
