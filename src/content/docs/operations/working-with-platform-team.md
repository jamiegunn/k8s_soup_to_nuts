...
**PDBs on everything that matters.** Upgrades drain nodes. A drain evicts your pods, and the *only* thing pacing those evictions is your PodDisruptionBudget. No PDB = all replicas can be evicted simultaneously if they share a node. Wrong PDB (`minAvailable` equal to replica count) = the drain wedges and the platform team pages *you*. Target: every multi-replica service has a PDB allowing at least one disruption *at its HPA floor* (`maxUnavailable: 1`, not a `minAvailable` equal to `minReplicas`), with `unhealthyPodEvictionPolicy: AlwaysAllow` so a crashlooping pod can't hold the drain. The six questions to ask them back — drain timeout, `--grace-period`, the kubelet's shutdown window, the calendar — and the window runbook are [The Maintenance Contract](/disruption/platform-contract/); the checklist is [High availability](/workloads/high-availability/).
