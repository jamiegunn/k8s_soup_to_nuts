...
- **Start** (you are here) — the mental model and the daily toolkit. Begin with [How Kubernetes Works](/start/how-kubernetes-works/) for the reconciliation model that pays for everything else, then [The Three Doors](/start/three-doors/) for the model that fits any workload you deploy, its sibling [The Three Lenses](/start/three-lenses/) for the model that fits any number you'll read about it, and [The Two Roads](/start/two-roads/) for the map you troubleshoot with when either stops being true; then follow [Learning Paths Track 1](/learning-paths/#1-new-to-deploying-on-kubernetes) for the full reading order through the rest of the section.
...
| Know where to look first when something is broken — which verb failed, which hop, which layer | [The Two Roads](/start/two-roads/) |
...
1. **Day 1** — run the access check above; set your default namespace; read [How Kubernetes Works](/start/how-kubernetes-works/), [The Three Doors](/start/three-doors/), [The Three Lenses](/start/three-lenses/), [The Two Roads](/start/two-roads/), and [Life of a Deployment](/start/life-of-a-deployment/).
