...
The prerequisites are platform territory: a CSI driver with snapshot support, the external-snapshotter controller, and a `VolumeSnapshotClass` — plus RBAC for you to create `VolumeSnapshot` objects. `kubectl get volumesnapshotclass` tells you if it's plumbed; if not, that's the ask ([CSI Drivers](/controllers/csi-drivers/) explains the machinery). For how snapshots and backups actually work inside one provider — Longhorn's in-cluster snapshot chain vs incremental backups to an S3/NFS backupstore — see [Longhorn Under Valkey §8](/architectures/valkey-longhorn-deep-dive/#8-backups-snapshots-and-dr-the-mechanics).
