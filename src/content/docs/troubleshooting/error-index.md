...
| `ScalingLimited True` ... `TooManyReplicas` / `at max replicas` | The HPA wants more but `maxReplicas` (or a quota) is the ceiling — not a bug | [HPA Not Scaling](/troubleshooting/hpa-not-scaling/#scalinglimited-true--you-hit-a-bound), [Capacity & Governance](/autoscaling/capacity-and-governance/) |
| `FailedScheduling` ... `Insufficient memory` right after a scale-up | The HPA asked for pods the fixed pool can't hold — Σ(maxReplicas × requests) exceeded allocatable/quota | [Capacity & Governance](/autoscaling/capacity-and-governance/), [Pod Pending](/troubleshooting/pod-pending/#1-insufficient-cpu-or-memory) |
| `ORA-00018: maximum number of sessions exceeded` (in app logs, during/after scale-out) | Scaling multiplied connection pools past Oracle's session budget — the ceiling math was skipped | [REST API + Oracle](/autoscaling/rest-api-oracle/) |
| `KEDAScalerFailed` / `error resolving auth params` / `keda_scaler_errors_total` rising | KEDA can't reach or authenticate to the scaler's source — replicas freeze at current count | [Messaging Consumers](/autoscaling/messaging-consumers/), [Getting the Metrics](/autoscaling/getting-the-metrics/) |
| ScaledObject `READY False` | The trigger's connection/auth/config is broken; `kubectl describe scaledobject` names the call that failed | [KEDA architecture](/architectures/keda-autoscaling/), [Messaging Consumers](/autoscaling/messaging-consumers/) |
