...
And the debugging tools you use are namespace verbs. `kubectl exec` doesn't SSH anywhere (the full contrast — protocol vs API-mediated streams — is [its own deep dive](/foundations/ssh/)): the kubelet asks the runtime to spawn your command with `setns()` into the target container's namespaces ([setns(2)](https://man7.org/linux/man-pages/man2/setns.2.html) — the same thing `nsenter` does by hand on a node). An [ephemeral debug container](/troubleshooting/debugging-toolbox/) is a new container joining the pod's *shared* namespaces — which is why `kubectl debug` can see the app's network but needs `--target` (pid-namespace sharing) to see its processes. And the `/proc/<pid>/root` trick from [the jattach deep dive](/java/jattach-deep-dive/) works because a process's mount namespace is reachable *through* procfs even when it isn't yours.

## cgroups: the budget

Namespaces change what a process *sees*; control groups ([cgroups(7)](https://man7.org/linux/man-pages/man7/cgroups.7.html), [kernel cgroup-v2 guide](https://docs.kernel.org/admin-guide/cgroup-v2.html), and the [cgroups deep dive](/foundations/cgroups/)) change what it *gets*. Every pod on a node lives in a kernel-maintained tree under `/sys/fs/cgroup`, and the tree's shape *is* your QoS class:
...
Two asymmetries worth engraving. **CPU is compressible** — blow the budget and you wait (throttled), which is why CPU limits cause latency, not crashes; the scheduler mechanics behind that sentence — vruntime, weights, and the burst-throttling pathology — are [CPU Scheduling and the CFS](/foundations/cpu-scheduling-and-cfs/). **Memory is not** — blow the budget and something dies. That single kernel fact generates most of the advice in [Requests, Limits, and the Knobs](/tuning/requests-limits-knobs/) and [Resources & QoS](/workloads/resources-and-qos/).
...
A container image is a stack of read-only tarballs; [overlayfs](https://docs.kernel.org/filesystems/overlayfs.html) is the kernel filesystem that makes the stack look like one root. (The whole storage stack under it — block devices, filesystems, bind mounts, the page cache, encryption — is [its own deep dive](/foundations/storage-and-filesystems/).) The image layers become `lowerdir`s, a fresh empty directory becomes your container's `upperdir`, and every write you make lands in the upper layer — copy-on-write, per container:
...
The pod's network namespace would be an isolated island without a cable out. (For the primitives themselves — interfaces, routes, bridges, and the packet's walk through the host — see [Linux Networking](/foundations/linux-networking/); for the hook-and-chain engine the rules below run on, [Firewalls: netfilter, iptables, and Beyond](/foundations/firewalls-and-netfilter/).) The CNI plugin provides it: a **veth pair** — two virtual NICs joined at the hip; packets in one end exit the other. One end becomes `eth0` *inside* the pod's net namespace, the other end sits in the node's namespace, wired into a bridge or the routing table. That's the whole "pod networking" trick at node scope; how the fabric spans nodes is [the networking model](/networking/networking-model/).
