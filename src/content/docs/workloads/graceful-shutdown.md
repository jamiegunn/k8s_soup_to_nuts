...
**Node drains and evictions.** A drain calls the Eviction API per pod; each granted eviction is a standard deletion — deletionTimestamp, preStop, SIGTERM, G, SIGKILL. Same sequence, same race, same budget. Your PDB controls *how many at once* and *whether now* ([PDBs, All the Way Down](/disruption/pod-disruption-budgets/)); the drain's own choreography — cordon, the concurrent eviction calls, the 429 retry loop, the platform's timeout — is [What a Drain Actually Does](/disruption/anatomy-of-a-drain/); this page controls whether each one is clean. This is why graceful shutdown is a pillar of [High Availability](/workloads/high-availability/): the platform team drains nodes weekly, and each drain is a rollout you didn't schedule. The one place the budget gets externally squeezed: kubelet graceful node shutdown on reboots and spot reclaims caps the grace it can grant to the node's configured shutdown window — if the platform's window is 30s, your `terminationGracePeriodSeconds: 120` is aspirational on those nodes ([When Nobody Asked](/disruption/involuntary-disruptions/#kubelet-graceful-node-shutdown) has the numbers to ask for).
