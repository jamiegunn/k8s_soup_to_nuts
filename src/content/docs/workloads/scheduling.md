...
- **`minDomains`** — with `DoNotSchedule`, treats fewer-than-N observed domains as skew violations. On a cluster with a cluster autoscaler this pressures it to open a new domain; on a **fixed on-prem pool there's no autoscaler to satisfy**, so the pod just stays `Pending` until an Nth domain has room — reach for it only when you'd genuinely rather wait than under-spread.
- **`matchLabelKeys: [pod-template-hash]`** — the rolling-update fix. Without it, the constraint counts old-ReplicaSet and new-ReplicaSet pods together, so mid-rollout the "even" math is computed over pods that are about to disappear, and you end with the new pods lopsided. This scopes counting to pods sharing your template hash, i.e. per-ReplicaSet. (Beta and on by default since 1.27 — on **older clusters the field is silently ignored**, taking the old-vs-new skew problem back with it, so confirm your cluster version before relying on it.)
